    ...
    },
    {
      ...
      "points": [
        ...
        },
        {
          "id": "java新特性-1-5-4",
          "tag": "Java 新特性",
          "desc": "8:Lambda/Stream/Optional/接口默认方法；9:模块化 JPMS/JShell；14:Record；15:密封类 Sealed；16:模式匹配 instanceof；17:LTS；21 LTS:虚拟线程 Virtual Thread/模式匹配 switch",
          "details": [
            {
              "id": "java新特性-1-5-4-d0",
              "tag": "JDK8",
              "desc": "Lambda表达式+Stream API+Optional+日期API+接口默认方法"
            },
            {
              "id": "java新特性-1-5-4-d1",
              "tag": "JDK9",
              "desc": "模块化系统jlink定制运行时+接口私有方法+多版本jar"
            },
            {
              "id": "java新特性-1-5-4-d2",
              "tag": "JDK11",
              "desc": "HttpClient标准化+var局部变量+Flight Recorder免费"
            },
            {
              "id": "java新特性-1-5-4-d3",
              "tag": "JDK17",
              "desc": "密封类sealed+Record类+模式匹配instanceof正式版"
            },
            {
              "id": "java新特性-1-5-4-d4",
              "tag": "JDK21",
              "desc": "虚拟线程(Virtual Thread)+模式匹配switch+字符串模板+序列化集合"
            }
          ]
        }
      ]
      ...
      "points": [
        {
          "id": "架构-5-2-0",
          "tag": "架构",
          "desc": "基于NIO异步事件驱动网络框架。核心:EventLoopGroup(事件循环组)+EventLoop(事件循环绑定Selector)+ChannelPipeline(管道链式Handler)+ByteBuf(字节缓冲池化)。BossGroup接收连接,WorkerGroup处理IO",
          "details": [
            {
              "id": "架构-5-2-0-d0",
              "tag": "EventLoopGroup->EventLoop->Pipeline->ByteBuf",
              "desc": "Netty核心架构：EventLoopGroup管理EventLoop组，EventLoop绑定Selector事件循环，Pipeline链式Handler处理，ByteBuf池化缓冲"
            }
          ]
        },
        {
          "id": "线程模型-5-2-1",
          "tag": "线程模型",
          "desc": "主从Reactor多线程。bossGroup(默认CPUx1)负责Accept事件;workerGroup(默认CPUx2)负责Read/Write事件;业务Handler在WorkerGroup执行(可加DefaultEventExecutorGroup业务线程池)。一个Channel绑定一个EventLoop(线程),保证无锁串行执行",
          "details": [
            {
              "id": "线程模型-5-2-1-d0",
              "tag": "BossGroup(CPUx1)+WorkerGroup(CPUx2)",
              "desc": "BossGroup默认1个线程负责Accept连接，WorkerGroup默认CPU核心数×2个线程处理IO读写，分工明确高效"
            },
            {
              "id": "线程模型-5-2-1-d1",
              "tag": "Channel绑定EventLoop线程安全",
              "desc": "每个Channel整个生命周期绑定一个EventLoop线程，所有IO操作在同一线程串行执行无需加锁保证线程安全"
            }
          ]
        },
        {
          "id": "netty零拷贝-5-2-2",
          "tag": "Netty零拷贝",
          "desc": "CompositeByteBuf(逻辑合并多个ByteBuf无需复制)、wrap()包装(共享底层数组)、slice()切片(共享数据)。OS层面:FileRegion.transferTo()使用sendfile系统调用(内核直接发送到网卡)",
          "details": [
            {
              "id": "netty零拷贝-5-2-2-d0",
              "tag": "CompositeByteBuf/wrap/slice零拷贝",
              "desc": "CompositeByteBuf逻辑合并多个ByteBuf无需物理复制，wrap共享底层数组，slice切片共享数据区域，应用层零拷贝"
            },
            {
              "id": "netty零拷贝-5-2-2-d1",
              "tag": "transferTo->sendfile系统调用",
              "desc": "FileRegion.transferTo()底层调用OS sendfile系统调用，数据从文件内核缓冲区直接发送到网卡缓冲区跳过用户空间"
            }
          ]
        },
        {
          "id": "心跳与空闲检测-5-2-3",
          "tag": "心跳与空闲检测",
          "desc": "IdleStateHandler:读超时/写超时/全部超时。超时触发IdleStateEvent配合UserEventHandler发送心跳包或关闭连接。防止半包死连接占用资源",
          "details": []
        },
        {
          "id": "拆包粘包解决-5-2-4",
          "tag": "拆包粘包解决",
          "desc": "TCP粘包拆包问题:LineBasedFrameDecoder(换行符分隔)/DelimiterBasedFrameDecoder(自定义分隔符)/FixedLengthFrameDecoder(固定长度)/LengthFieldBasedFrameDecoder(长度字段,最常用生产环境)",
          "details": [
            {
              "id": "拆包粘包解决-5-2-4-d0",
              "tag": "推荐LengthFieldBasedFrameDecoder",
              "desc": "LengthFieldBasedFrameDecoder按消息头长度字段解码最灵活生产首选，可配置长度字段偏移、长度和偏移调整"
            }
          ]
        }
      ]
